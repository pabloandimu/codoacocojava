...
desplegarListado  = () => {
    if (!confirm("Desea ver su REGISTRO?")) {
        return;
    }
    mostrarListado();
}    
...
document
  .getElementById("btnGetUsers")
  .addEventListener("click", listarOradores);
...
const getOradorSeleccionado = () => {
  // funcion cortita, solo sabe ir a buscar en el local storage y devolver
...
const actualizarOrador = () => {
  ...
  //debo enviar estos datos al sevidor: https://www.freecodecamp.org/espanol/news/tutorial-de-fetch-api-en-javascript-con-ejemplos-de-js-fetch-post-y-header/
  fetch(
    `http://localhost:8080/web-app-23544/api/orador?id=${oradorSeleccionado.id}`,
    {
      // le agrego como parámetro el id $
      method: "PUT",
      body: JSON.stringify(orador),
    }
  )
